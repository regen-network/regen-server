...
export const account = pgTable('account', {
  ...
  nonce: text('nonce')
    .default(sql`md5(gen_random_bytes(256))`)
    .notNull(),
});
...
export const creditClass = pgTable(
  'credit_class',
  {
    id: uuid('id')
      .default(sql`uuid_generate_v1()`)
  ...
  table => {
    return {
      onChainIdKey: uniqueIndex('credit_class_on_chain_id_key').on(
        table.onChainId,
      ),
...
export const creditClassVersion = pgTable(
  'credit_class_version',
  {
    id: uuid('id')
      .default(sql`uuid_generate_v1()`)
      .notNull()
      .references(() => creditClass.id),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' })
      .defaultNow()
      .notNull(),
...
export const creditBatch = pgTable(
  'credit_batch',
  {
    ...
    creditClassVersionCreatedAt: timestamp('credit_class_version_created_at', {
      withTimezone: true,
      mode: 'string',
    }),
    metadata: jsonb('metadata'),
  ...
  table => {
    return {
      batchDenomKey: uniqueIndex('credit_batch_batch_denom_key').on(
        table.batchDenom,
      ),
      creditClassVersionIdCreditClassVersionIdx: index(
        'credit_batch_credit_class_version_id_credit_class_version_idx',
      ).on(table.creditClassVersionId, table.creditClassVersionCreatedAt),
      ...
      creditBatchCreditClassVersionIdCreditClassVersioFkey: foreignKey({
        columns: [
          table.creditClassVersionId,
          table.creditClassVersionCreatedAt,
        ],
...
export const project = pgTable(
  ...
  table => {
    return {
      ...
      adminWalletIdIdx: index('project_admin_wallet_id_idx').on(
        table.adminWalletId,
      ),
      creditClassIdIdx: index('project_credit_class_id_idx').on(
        table.creditClassId,
      ),
