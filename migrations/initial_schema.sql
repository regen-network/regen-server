...
BEGIN
    ...
    COMMENT ON ROLE app_user IS 'This is the user role that all logged in users inherit from and should be used when creating role-level security policies.';
EXCEPTION
...
BEGIN
    ...
    ALTER ROLE auth_user WITH NOSUPERUSER INHERIT NOCREATEROLE NOCREATEDB NOLOGIN NOREPLICATION NOBYPASSRLS;
    COMMENT ON ROLE auth_user IS 'This is the user role that the keplr-based login system uses.';
    GRANT app_user TO auth_user GRANTED BY postgres;
EXCEPTION
    WHEN duplicate_object THEN
        RAISE NOTICE '%, skipping', SQLERRM USING ERRCODE = SQLSTATE;
END$$;
